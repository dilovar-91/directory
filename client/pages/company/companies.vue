...
<yandex-map 
  ...
  <section class="py-5 bg-gray-100 shadow">
      <div class="container">
        ...
        <p class="lead mb-5">Все список </p>
        <p class="ml-2 lead text-danger font-weight-bold" v-if="companies.total <= 0" >Организации по категории не существуют.</p>
        <h5>{{$t('category')}}</h5>
        <ul class="nav nav-pills-custom" >
          <li class="nav-item" v-for="category in categories" :key="category.id">
            <router-link :to="'/companies/?category='+category.id" class="nav-link active">{{category.name}}</router-link>
          </li>          
        </ul>
      </div>
    </section>    
    <section class="py-5">
      <div class="container" v-if="companies.total > 0">
        <div class="d-flex justify-content-between align-items-center flex-column flex-md-row mb-4">
          <div class="mr-3">
            <p class="mb3- mb-md-0" >Показано <strong class="text-danger">{{(companies.current_page-1)*companies.per_page+1}}-{{ ( to > companies.total ?  companies.total: to )}} из {{companies.total}}</strong> организации по городу <strong class="text-primary">""</strong></p>
          </div>
        ...
        <list-pagination  :page="this.page || 1" :max-page="maxPage" :route="'companies'" :sort="sortSelected" />
      </div>
    </section>
</div>
</template>
...
layout: "main", 
  ...
  head () {
    ...
    meta: [
        { hid: 'description', name: 'description', content: 'dgmrknjtg' }
    ],
    }
  },
  ...
  async fetch({store, error, params, query}) { 
    await Promise.all([
     store.dispatch("company/get", {page: params.page || 1, sort: params.sort || '', category: query.category || ''}),    
